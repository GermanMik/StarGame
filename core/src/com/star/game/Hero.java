...
//        batch.setColor(1, 1, 1, 1);
    ...
    public void update(float dt) {
        ...
        if (GameScreen.isAndroid) {
            ...
            if (mgip.isTouchedInArea(1280 - 256 - 20, 50, 256, 256) > -1) {
                tryToFire(dt);
            }
            if (joystick.touched) {
        ...
        ParticleEmitter.getInstance().setupParticle(position.x - (float)cos(angle) * 24f, position.y - (float)sin(angle) * 24f, (float)(Math.random() - 0.5) * 30, (float)(Math.random() - 0.5) * 30, 0.3f, size, 0.1f, 1, 1, 1, 1, 0, 0, 1, 0f);
        if(reddish > 0.8f) {
            ParticleEmitter.getInstance().setupParticle(position.x, position.y , (float)(Math.random() - 0.5) * 250, (float)(Math.random() - 0.5) * 250, 0.8f, 1.8f, 1.2f, 1, 0, 0, 1, 1, 1, 0, 0f);
        }
    }
