...
public class MenuScreen implements Screen {
    ...
    public void show() {
        ...
        startGameBtnTexture = Assets.getInstance().mainAtlas.findRegion("play");
        endGameBtnTexture = Assets.getInstance().mainAtlas.findRegion("exit");
    ...
    public void render(float delta) {
        ...
        batch.draw(startGameBtnTexture, 400, 300, 550, 313);
        batch.draw(endGameBtnTexture, 500, 150, 254, 74);
        batch.end();
    }
    ...
    public void update(float dt) {
        ...
        MyGameInputProcessor mgip = (MyGameInputProcessor) Gdx.input.getInputProcessor();
        if (mgip.isTouchedInArea(400, 300, 550, 313) > -1) {
            ScreenManager.getInstance().switchScreen(ScreenManager.ScreenType.GAME);
        }
        if (mgip.isTouchedInArea(500, 150, 254, 74) > -1) {
            Gdx.app.exit();
        }
    }
